...
// Fixed text positions for certificate
const TEXT_CONFIG: Record<string, TextConfig> = {
  name: { x: 873, y: 570, fontSize: 40, fontFamily: "GoogleSans", fontWeight: "700", color: "#000", align: "center" },
  date: { x: 494, y: 989, fontSize: 16, fontFamily: "GoogleSans", fontWeight: "500", color: "#000", align: "center" },
}
...
export default function CertificateEditor({ template, name, onDownload }: CertificateEditorProps) {
  ...
  useEffect(() => {
    ...
    const loadFont = async (font: typeof fonts[0]) => {
      try {
        ...
        console.warn("Failed to load local font, falling back to Google Fonts:", font.name)
      }
    }
  ...
  useEffect(() => {
    if (!fontsLoaded || !imageLoaded || !canvasRef.current) return
